...
main(int argc, const char** argv)
{
    ...
    // create window
    window = glfwCreateWindow(width, height, "Textures and Cameras", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		const vec3 eye = vec3(sinf(timeValue * cameraSpeed) * cameraDistance, 1, cosf(timeValue * cameraSpeed) * cameraDistance);
